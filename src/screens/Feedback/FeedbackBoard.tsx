    ...
    <div className="h-full flex flex-col">
      ...
      {editOpen && (
        <Modal
          ...
          title={editMode === "create" ? "새 글 작성" : "글 수정"}
          maxWidth="max-w-[560px]"
        >
          <input
            className="w-full ui-input mb-2"
            placeholder="제목"
            value={editTitle}
            onChange={(e) => setEditTitle(e.target.value)}
          />
          <textarea
            className="w-full ui-input h-40 mb-3"
            placeholder="내용"
            value={editContent}
            onChange={(e) => setEditContent(e.target.value)}
          />
          <div className="flex justify-end gap-2">
            <button className="px-3 py-2 rounded-xl border border-white/10 hover:bg-white/10" onClick={() => setEditOpen(false)}>취소</button>
            {editMode === "create" ? (
              <button
